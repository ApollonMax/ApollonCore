...
// Copyright (c) 2016-2018 The Apollon developers
    ...
    while (pcursor->Valid()) {
        ...
        try {
            ...
            if (chType == 'b') {
                ...
                if (pindexNew->nHeight <= Params().LAST_POW_BLOCK() || (pindexNew->nHeight >= 5000 && pindexNew->nHeight <= 5010)) {
                ...
                if(pindexNew->nAccumulatorCheckpoint != 0 && pindexNew->nAccumulatorCheckpoint != nPreviousCheckpoint) {
                    //Don't load any checkpoints that exist before v2 zxap. The accumulator is invalid for v1 and not used.
